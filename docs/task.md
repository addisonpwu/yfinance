...
3. **原始文件保護**：`stock_news.json`、`aastocks_news.json`、`eastmoney_hk_news.json`、`sinafinance_hk_news.json` **絕對不可刪除、移動或修改**。
4. **僅處理港股相關新聞**。
...
3. ***清理執行過程中生成的臨時檔案***（注意：保留原始文件 stock_news.json、aastocks_news.json、eastmoney_hk_news.json、sinafinance_hk_news.json）
