...
專案整合了豐富的技術指標，用於更精確的市場分析：

| 指標類別 | 指標名稱 | 說明 |
|---------|---------|------|
| 動量指標 | RSI, CMO, Williams %R, Stochastic %K/D | 衡量價格動能與超買超賣 |
| 波動率指標 | ATR, ATR%, BBP, 年化波動率 | 評估市場波動程度 |
| 趨勢指標 | MACD, 多周期 MA | 識別趨勢方向與強度 |
| 量能指標 | Volume MA, Volume Z-Score, Volume Change | 分析成交量異常 |

### 策略信號評分系統

專案提供多維度信號評分系統，綜合評估交易機會：

- **趨勢信號 (25%)**: 均線排列、價格相對位置
- **動量信號 (20%)**: 價格突破、RSI/MACD 狀態
- **量能確認 (15%)**: 成交量突破、量價配合
- **大盤環境 (20%)**: 市場健康狀態、相對表現
- **形態強度 (20%)**: 技術形態識別
...
- **回測指標**: 夏普比率、索提諾比率、卡瑪比率、最大回撤、勝率、盈虧比
- **蒙特卡洛測試**: 評估策略穩健性，輸出置信區間
- **Walk-Forward 驗證**: 樣本外測試，防止過擬合
- **交易成本模擬**: 手续费、滑點建模

### 動態倉位管理

根據風險承受度和市場狀況，自動計算最佳倉位：
...
-   **2026-02-09**:
    -   **技術指標增強 (Technical Indicators)**: 新增 BBP (布林帶位置百分位)、CMO (錢德動量指標)、Williams %R、Stochastic %K/%D、Volume Z-Score、ATR% 等技術指標。
    -   **策略信號評分系統 (Signal Scorer)**: 新增多維度信號評分器，綜合趨勢、動量、量能、大盤環境、形態強度五個維度計算綜合得分。
    -   **市場環境識別 (Market Regime)**: 新增市場環境識別策略，可識別趨勢市場、震盪市場、高波動市場。
    -   **回測驗證系統 (Backtest Engine)**: 新增完整回測引擎，包含蒙特卡洛測試和 Walk-Forward 驗證，以及夏普比率、索提諾比率、卡瑪比率等指標計算。
    -   **動態倉位管理 (Position Sizer)**: 新增動態倉位計算器，支持 Kelly 公式、波動率調整、信心加權、風險平價等多種倉位策略。
    -   **Finviz 數據整合**: 新增 Finviz 數據源，可獲取估值比率、分析師評級、目標價、內部交易等額外數據。
