...
-   `data_loader/`: 負責從不同來源獲取股票代碼列表。
    -   `us_loader.py`: 負責獲取美股代碼列表
    -   `hk_loader.py`: 負責獲取港股代碼列表
    -   `get_yahoo_news.py`: 用於獲取和解析雅虎財經股票新聞的工具
...
此策略經過重構，旨在更精準地捕捉大型機構投資者在拉升前，於市場上悄悄吸納籌碼的「潛伏」階段。

-   **波動率收縮**: 透過布林帶寬度指標，要求股票在信號出現前，必須處於低波動的盤整蓄勢狀態。
-   **資金持續流入**: 結合OBV指標的斜率與均線，雙重確認有資金在持續淨流入。
-   **量能健康**: 尋找「量能萎縮後，溫和放量」的特徵，過濾掉爆發天量所帶來的潛在分歧或出貨風險。
-   **趨勢受控**: 要求股價處於健康的長期趨勢之上，但又沒有離長期成本區過遠，形態穩健。
...
此策略旨在捕捉下降趨勢中的潛在轉折點，專門尋找「牛市中的回檔結束點」，勝率更高。

-   **經典形態**：嚴格篩選「下跌孕線 (Harami)」形態，即一根黑K母線完全吞噬次日的K線。
-   **雙重趨勢確認**：要求形態出現在短期下跌趨勢中，但股價必須仍在長期上升趨勢（200日均線）之上。
-   **信號確認**：結合RSI超賣、成交量萎縮，並在形態完成後，等待一根放量的紅K向上突破，作為最終的入場信號。

### 6. 布林帶擠壓突破 (BollingerSqueezeStrategy)
此策略旨在捕捉由長期盤整、波動率極度壓縮的狀態，轉變為趨勢行情（主升浪）的起爆點。

-   **持續擠壓**：透過布林帶寬度指標，量化識別出波動率已持續收縮一段時間的股票。
-   **趨勢健康度**：確保長期趨勢（200日均線）本身是平穩或向上的，避免在熊市中操作。
-   **突破品質**：要求突破必須是放量的、實體飽滿的紅K棒，以確認突破的強度與決心。
...
-   **新聞提取功能**：透過 `data_loader/get_yahoo_news.py` 模組，可以提取特定股票的相關新聞標題和連結，使用靈活的正則表達式匹配模式，支援所有港股代碼格式。
-   **動態策略加載**：分析引擎會自動偵測並執行 `strategies` 文件夾中的所有策略。
-   **豐富的結果輸出**：
    -   **詳細報告**：程式會生成一份詳細報告檔案 (`..._details.txt`)，其中包含每支入選股票的基本面資訊（市值、市盈率等）和最新的相關新聞，方便進行深度分析。
    -   **簡潔摘要**：同時，原有的逗號分隔格式 (`NASDAQ:AAPL,NYSE:MSFT`) 的摘要列表也會被儲存到對應市場的 `.txt` 檔案中，方便直接複製貼上至其他軟體使用。
...
-   **2025-10-07**:
    -   **功能擴充 (基本面與新聞)**: 分析結果現在包含豐富的基本面數據（公司資訊、估值指標）和近期相關新聞，並會生成一份獨立的詳細報告檔案 (`_details.txt`)。
    -   **數據獲取優化**: 將歷史數據的獲取週期改為「全部歷史數據 (max)」，並在分析時顯示獲取的數據條數。
    -   **錯誤修復**: 修復了因部分股票缺少財務數據（如市盈率）而導致程式在生成報告時崩潰的 `ValueError`。
    -   **程式碼維護**: 修正了 `vcp_pocket_pivot_strategy` 中一個由 pandas 版本更新引起的 `FutureWarning`。
