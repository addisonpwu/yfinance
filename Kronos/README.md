...
**Advanced: Getting Raw Samples**

By default, if `sample_count` is greater than 1, the `predict` method returns a single forecast representing the average of all generated sample paths. To access the raw, individual sample paths for more advanced statistical analysis (e.g., calculating custom probability distributions), you can use the `return_samples=True` parameter.

```python
# Generate all raw prediction samples
all_samples_df = predictor.predict(
    ...,
    sample_count=100,
    return_samples=True 
)
```
When `return_samples=True`, the method returns a wide DataFrame where columns for each sample are appended with a suffix (e.g., `close_0`, `volume_0`, `close_1`, `volume_1`, etc.).
...
此腳本用於設定 Kronos 專案的開發環境。它會執行以下操作：

1.  **建立虛擬環境**：在專案根目錄下建立一個名為 `venv` 的 Python 虛擬環境。
2.  **啟用虛擬環境**：啟用 `venv` 虛擬環境。
3.  **安裝相依套件**：從 `requirements.txt` 和 `webui/requirements.txt` 安裝所有必要的 Python 相依套件。
...
`scripts/` 目錄包含用於執行特定任務的 Python 腳本。

#### `download_us_stocks.py`

此腳本用於下載美國股市的歷史資料。參考https://github.com/shiyu-coder/Kronos-demo/blob/master/update_predictions.py

#### `prediction_hk.py`

此腳本用於預測香港股市的股價。您可以透過修改 `scripts/config/config_hk.py` 檔案來設定要預測的股票代碼。

**使用方法：**

```shell
python scripts/prediction_hk.py
```
...
本專案的詳細開發與更新紀錄請參閱 [CHANGELOG.md](CHANGELOG.md)（增量記錄）。
