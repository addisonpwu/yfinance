...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False):
    with torch.no_grad():
        ...
        preds = z.cpu().numpy()
        # Return the full array of samples, do not average here.
        # The shape is (batch_size, sample_count, seq_len, features)
        return preds
...
class KronosPredictor:
    ...
    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True, return_samples=False):
        ...
        # preds shape is (1, sample_count, max_context, features)
        preds = self.generate(x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose)
        # Slice to get only the predicted part
        preds = preds[:, :, -pred_len:, :]

        if return_samples:
            # Denormalize all samples
            # Squeeze batch dimension, shape becomes (sample_count, pred_len, features)
            all_samples_denorm = preds.squeeze(0) * (x_std + 1e-5) + x_mean
            
            # Create a combined dataframe with mangled column names
            cols = self.price_cols + [self.vol_col, self.amt_vol]
            dfs_to_concat = []
            for i in range(all_samples_denorm.shape[0]):
                sample_df = pd.DataFrame(all_samples_denorm[i, :, :], columns=[f"{col}_{i}" for col in cols], index=y_timestamp)
                dfs_to_concat.append(sample_df)
            
            combined_df = pd.concat(dfs_to_concat, axis=1)
            return combined_df
        else:
            # Original behavior: average and return one dataframe
            preds_mean = np.mean(preds, axis=1) # Average over samples
            preds_mean = preds_mean.squeeze(0) # Squeeze batch dimension
