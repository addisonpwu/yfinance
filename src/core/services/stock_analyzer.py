...
from src.core.services.ai_analysis_persister import AIAnalysisPersister
...
class StockAnalyzer:
    ...
    def _extract_technical_indicators(hist: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if all(
                col in hist.columns
                for col in ["BB_Upper", "BB_Lower", "BB_Middle", "BBP"]
            ):
            ...
            if pd.notna(close):
                ...
                if ma_positions:
                    ...
                    above_count = sum(
                        1
                        for k, v in ma_positions.items()
                        if k.startswith("above_ma_") and v
                    )
    ...
    def _apply_rate_limit(self):
        ...
        with StockAnalyzer._rate_limit_lock:
            ...
            if StockAnalyzer._consecutive_errors > 0:
                delay = min(
                    self.base_delay * (2**StockAnalyzer._consecutive_errors),
                    self.max_delay,
                )
            else:
                # 正常情况下使用基础延迟 + 随机抖动
    ...
    def _on_success(self):
        """成功时重置错误计数"""
        with StockAnalyzer._rate_limit_lock:
            StockAnalyzer._consecutive_errors = max(
                0, StockAnalyzer._consecutive_errors - 1
            )

    def _on_error(self):
        """错误时增加错误计数"""
        with StockAnalyzer._rate_limit_lock:
            StockAnalyzer._consecutive_errors = min(
                StockAnalyzer._consecutive_errors + 1, 5
            )
    ...
    def _analyze_with_provider(
        ...
        try:
            ...
            result = ai_service.analyze_stock(
                stock_data, hist, interval=interval, model=model
            )
            if result:
                return {
                    "provider": provider,
    ...
    def analyze(
        ...
        try:
            # 获取股票数据
            hist = self.data_repo.get_historical_data(
                symbol, market, interval=interval, force_refresh=force_refresh
            )
            ...
            if (
                ...
                or len(hist) < 2
                or info is None
                or (isinstance(info, dict) and len(info) == 0)
            ):
            ...
            try:
                ...
                if len(providers_to_use) > 1:
                    ...
                    if ai_results:
                        ...
                        # 计算平均置信度
                        avg_confidence = (
                            total_confidence / len(ai_results) if ai_results else 0.5
                        )
                        ...
                        persister = AIAnalysisPersister()
                        persister.persist_multi_provider_results(
                            symbol=symbol,
                            interval=interval,
                    ...
                    # 单提供商分析
                    ai_result = self.ai_service.analyze_stock(
                        stock_data, hist, interval=interval, model=model
                    )
                    if ai_result:
                        ai_analysis = {
                            ...
                            ai_results=[
                                {
                                    ...
                                    if hasattr(ai_result, "detailed_analysis")
                                    else None,
                                }
                            ],
                        )
