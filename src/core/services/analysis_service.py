...
def run_analysis(
    ...
    try:
        ...
        print(f"⚠️  数据库初始化失败: {db_err}")

    # --- 缓存版本检查 ---
    is_sync_needed, status_msg = cache_manager.check_version(
        market, force_fast_mode, interval
    )
    print(status_msg)
    ...
    if not strategies_to_run:
        ...
        return []
    print(
        f"已加载 {len(strategies_to_run)} 个策略: {[s.name for s in strategies_to_run]}"
    )
    ...
    def analyze_single_stock(symbol: str) -> StockAnalysisResult:
        ...
        if result.success and result.strategies:
            ...
            if result.ai_analysis:
                print(
                    f"   🤖 AI 分析: 已完成 (模型: {result.ai_analysis.get('model_used', 'N/A')})"
                )
            else:
                print(f"   🤖 AI 分析: 未能完成")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_symbol = {
            executor.submit(analyze_single_stock, symbol): symbol for symbol in tickers
        }
...
def run_analysis_from_json(
    ...
    for i, symbol in enumerate(tickers, 1):
        try:
            # 强制刷新数据，获取最新数据
            hist = data_repo.get_historical_data(
                symbol, market, interval=interval, force_refresh=True
            )
            if hist is not None and not hist.empty:
                downloaded_count += 1
                print(
                    f"\r{' ' * 80}\r📥 [{i}/{len(tickers)}] {symbol} 数据已更新 ({len(hist)} 条记录)"
                )
            else:
            ...
            failed_downloads.append(symbol)
            print(
                f"\r{' ' * 80}\r❌ [{i}/{len(tickers)}] {symbol} 下载失败: {str(e)[:50]}"
            )
